...
public:	
	...
	float DetectionDistance;//maximum distance to wanted actor that can be get spotted
...
private:
	...
	void UpdateClosest();//Checks if there's a new closest or farthest actor and notify's them
	...
	void DrawSingleLine(int i, FColor colllor);//Draws a line from this actor to a specyfic wanted actor
	
	void PrintList();//Shows the list of actors on screen

	TArray<AActor*> CaF;//Closest and Farthest
